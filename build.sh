...
if [ "$ARCH" == "jetson" ]; then
    docker build -f Dockerfile.jetson -t roboflow-area_detector-web .
elif [ "$ARCH" == "amd64" ]; then
    docker build -f Dockerfile.amd64 -t roboflow-area_detector-web .
fi

# Check if the build was successful
if [ $? -eq 0 ]; then
    echo "Successfully built Docker image 'roboflow-area_detector-web' for $ARCH."
else
