...
if [ "$ARCH" == "jetson" ]; then
    docker build -f Dockerfile.jetson -t roboflow-roi-web .
elif [ "$ARCH" == "amd64" ]; then
    docker build -f Dockerfile.amd64 -t roboflow-roi-web .
fi

# Check if the build was successful
if [ $? -eq 0 ]; then
    echo "Successfully built Docker image 'roboflow-roi-web' for $ARCH."
else
